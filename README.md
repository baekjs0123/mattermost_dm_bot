...
```text
/dm공지 등록 https://meeting.ssafy.com/hooks/본인의웹훅주소
/dm공지 등록확인
/dm공지 등록삭제
/dm공지 @user1 @user2 보낼 메시지
/dm공지 도움말
```
...
1. 사용자가 Mattermost에서 `/dm공지` 명령어를 입력합니다.
...
```text
/dm공지 등록 https://meeting.ssafy.com/hooks/xxxxxxxx
```
...
사용자가 `/dm공지 등록 ...` 명령어로 등록한 정보가 자동으로 저장됩니다.
...
```text
/dm공지 등록 https://meeting.ssafy.com/hooks/abc123xyz789
```
...
```text
/dm공지 등록 https://meeting.ssafy.com/hooks/새로운웹훅주소
```
...
```text
/dm공지 @받는사람1 @받는사람2 보낼 메시지
```

예시:

```text
/dm공지 @user1 @user2 오늘 14시까지 출결 사유를 입력해 주세요.
```
...
```text
/dm공지 @user1 @user1 @user2 회의가 시작됩니다.
```
...
Slash Command는 운영자가 한 번 생성합니다. 일반 사용자는 같은 `/dm공지` 명령어를 함께 사용합니다.
...
ngrok이 종료되면 Mattermost에서 로컬 서버에 접근할 수 없으므로 `/dm공지` 명령어도 동작하지 않습니다.
...
2. /dm공지 등록 https://meeting.ssafy.com/hooks/본인의웹훅주소
3. /dm공지 등록확인
4. /dm공지 @대상자 테스트 메시지
```
...
```text
/dm공지 @user1 @user2 오늘 14시까지 제출해 주세요.
```
